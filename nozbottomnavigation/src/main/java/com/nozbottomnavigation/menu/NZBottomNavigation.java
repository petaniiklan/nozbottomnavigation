...
import com.nozbottomnavigation.menu.notification.NZNotification;
import com.nozbottomnavigation.menu.notification.NZNotificationHelper;
 ...
 */
public class NZBottomNavigation extends FrameLayout {
	...
	private ArrayList<NZBottomNavigationItem> items = new ArrayList<>();
	private ArrayList<View> views = new ArrayList<>();
	private NZBottomNavigationBehavior<NZBottomNavigation> bottomNavigationBehavior;
	...
	private List<NZNotification> notifications = NZNotification.generateEmptyList(MAX_ITEMS);
	 ...
	 * Constructors
	 */
	public NZBottomNavigation(Context context) {
		super(context);
		init(context, null);
	}

	public NZBottomNavigation(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(context, attrs);
	}

	public NZBottomNavigation(Context context, AttributeSet attrs, int defStyleAttr) {
	...
	private void init(Context context, AttributeSet attrs) {
		...
		if (attrs != null) {
			TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.NZBottomNavigationBehavior_Params, 0, 0);
			try {
				selectedBackgroundVisible = ta.getBoolean(R.styleable.NZBottomNavigationBehavior_Params_selectedBackgroundVisible, false);
				translucentNavigationEnabled = ta.getBoolean(R.styleable.NZBottomNavigationBehavior_Params_translucentNavigationEnabled, false);
				
				titleColorActive = ta.getColor(R.styleable.NZBottomNavigationBehavior_Params_accentColor,
						ContextCompat.getColor(context, R.color.colorBottomNavigationAccent));
				titleColorInactive = ta.getColor(R.styleable.NZBottomNavigationBehavior_Params_inactiveColor,
						ContextCompat.getColor(context, R.color.colorBottomNavigationInactive));
				itemDisableColor = ta.getColor(R.styleable.NZBottomNavigationBehavior_Params_disableColor,
						ContextCompat.getColor(context, R.color.colorBottomNavigationDisable));
				
				coloredTitleColorActive = ta.getColor(R.styleable.NZBottomNavigationBehavior_Params_coloredActive,
						ContextCompat.getColor(context, R.color.colorBottomNavigationActiveColored));
				coloredTitleColorInactive = ta.getColor(R.styleable.NZBottomNavigationBehavior_Params_coloredInactive,
						ContextCompat.getColor(context, R.color.colorBottomNavigationInactiveColored));
				
				colored = ta.getBoolean(R.styleable.NZBottomNavigationBehavior_Params_colored, false);
	...
	private void createClassicItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			NZBottomNavigationItem item = items.get(itemIndex);
			...
			if (itemsEnabledStates[i]) {
				...
				iconDrawable = forceTint ? NZHelper.getTintDrawable(items.get(i).getDrawable(context),
	...
	private void updateItems(final int itemIndex, boolean useCallback) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (i == itemIndex) {
				...
				icon.setSelected(true);
				NZHelper.updateTopMargin(icon, inactiveMarginTop, activeMarginTop);
				NZHelper.updateLeftMargin(notification, notificationInactiveMarginLeft, notificationActiveMarginLeft);
				NZHelper.updateTextColor(title, itemInactiveColor, itemActiveColor);
				NZHelper.updateTextSize(title, inactiveSize, activeSize);
				if (forceTint) {
					NZHelper.updateDrawableColor(context, items.get(itemIndex).getDrawable(context), icon,
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP && colored) {
					...
					NZHelper.updateViewBackgroundColor(this, currentColor,
				...
				icon.setSelected(false);
				NZHelper.updateTopMargin(icon, activeMarginTop, inactiveMarginTop);
				NZHelper.updateLeftMargin(notification, notificationActiveMarginLeft, notificationInactiveMarginLeft);
				NZHelper.updateTextColor(title, itemActiveColor, itemInactiveColor);
				NZHelper.updateTextSize(title, activeSize, inactiveSize);
				if (forceTint) {
					NZHelper.updateDrawableColor(context, items.get(currentItem).getDrawable(context), icon,
	...
	private void updateSmallItems(final int itemIndex, boolean useCallback) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			if (i == itemIndex) {
				...
				if (titleState != TitleState.ALWAYS_HIDE) {
					NZHelper.updateTopMargin(icon, inactiveMargin, activeMarginTop);
					NZHelper.updateLeftMargin(notification, notificationInactiveMarginLeft, notificationActiveMarginLeft);
					NZHelper.updateTopMargin(notification, notificationInactiveMarginTop, notificationActiveMarginTop);
					NZHelper.updateTextColor(title, itemInactiveColor, itemActiveColor);
					NZHelper.updateWidth(container, notSelectedItemWidth, selectedItemWidth);
				}

				NZHelper.updateAlpha(title, 0, 1);
				if (forceTint) {
					NZHelper.updateDrawableColor(context, items.get(itemIndex).getDrawable(context), icon,
							itemInactiveColor, itemActiveColor, forceTint);
				}
				...
				if (titleState != TitleState.ALWAYS_HIDE) {
					NZHelper.updateTopMargin(icon, activeMarginTop, inactiveMargin);
					NZHelper.updateLeftMargin(notification, notificationActiveMarginLeft, notificationInactiveMarginLeft);
					NZHelper.updateTopMargin(notification, notificationActiveMarginTop, notificationInactiveMarginTop);
					NZHelper.updateTextColor(title, itemActiveColor, itemInactiveColor);
					NZHelper.updateWidth(container, selectedItemWidth, notSelectedItemWidth);
				}

				NZHelper.updateAlpha(title, 1, 0);
				if (forceTint) {
					NZHelper.updateDrawableColor(context, items.get(currentItem).getDrawable(context), icon,
							itemActiveColor, itemInactiveColor, forceTint);
				}
	...
	private void updateNotifications(boolean updateStyle, int itemPosition) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			final NZNotification notificationItem = notifications.get(i);
			final int currentTextColor = NZNotificationHelper.getTextColor(notificationItem, notificationTextColor);
			final int currentBackgroundColor = NZNotificationHelper.getBackgroundColor(notificationItem, notificationBackgroundColor);
			...
			if (updateStyle) {
				...
				if (notificationBackgroundDrawable != null) {
					...
					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
						notification.setBackground(NZHelper.getTintDrawable(defautlDrawable,
								currentBackgroundColor, forceTint));
					} else {
						notification.setBackgroundDrawable(NZHelper.getTintDrawable(defautlDrawable,
								currentBackgroundColor, forceTint));
					}
				}
			}
	 ...
	 */
	public void addItemAtIndex(int index, NZBottomNavigationItem item) {
	 ...
	 * Add an item
	 */
	public void addItem(NZBottomNavigationItem item) {
		if (this.items.size() > MAX_ITEMS) {
			Log.w(TAG, "The items list should not have more than 5 items");
		}
		items.add(item);
	 ...
	 * Add all items
	 */
	public void addItems(List<NZBottomNavigationItem> items) {
	 ...
	 */
	public NZBottomNavigationItem getItem(int position) {
	...
	public void setBehaviorTranslationEnabled(boolean behaviorTranslationEnabled) {
		...
		if (getParent() instanceof CoordinatorLayout) {
			...
			if (bottomNavigationBehavior == null) {
				bottomNavigationBehavior = new NZBottomNavigationBehavior<>(behaviorTranslationEnabled, navigationBarHeight);
			} else {
	...
	/**
	 * Manage the floating action button behavior with NZBottomNavigation
	...
	public void manageFloatingActionButtonBehavior(FloatingActionButton fab) {
		if (fab.getParent() instanceof CoordinatorLayout) {
			NZBottomNavigationFABBehavior fabBehavior = new NZBottomNavigationFABBehavior(navigationBarHeight);
	...
	public void setNotification(int nbNotification, int itemPosition) {
        ...
        notifications.set(itemPosition, NZNotification.justText(title));
     ...
     * @param itemPosition int
     */
	public void setNotification(NZNotification notification, int itemPosition) {
		if (itemPosition < 0 || itemPosition > items.size() - 1) {
            throw new IndexOutOfBoundsException(String.format(Locale.US, EXCEPTION_INDEX_OUT_OF_BOUNDS, itemPosition, items.size()));
		}
        if (notification == null) {
            notification = new NZNotification(); // instead of null, use empty notification
        }
